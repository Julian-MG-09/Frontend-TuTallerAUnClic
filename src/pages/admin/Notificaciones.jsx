...
const MOCK_NOTIFICACIONES = [
  { id: 1, titulo: 'Nueva cita',         mensaje: 'Tienes una nueva cita para el 2025-03-10', leida: false, fecha: '2025-03-09 08:30', usuario: 'maria_e' },
  { id: 2, titulo: 'Estado actualizado', mensaje: 'Tu cita fue confirmada',                   leida: true,  fecha: '2025-03-09 09:00', usuario: 'carlos123' },
  { id: 3, titulo: 'Nueva cita',         mensaje: 'Tienes una nueva cita para el 2025-03-12', leida: false, fecha: '2025-03-10 14:00', usuario: 'maria_e' },
  { id: 4, titulo: 'Estado actualizado', mensaje: 'Tu cita fue cancelada',                    leida: true,  fecha: '2025-03-11 10:00', usuario: 'admin01' },
]
...
export default function Notificaciones() {
  ...
  const filtered = notificaciones.filter(n => {
    const matchSearch = `${n.titulo} ${n.mensaje} ${n.usuario}`.toLowerCase().includes(search.toLowerCase())
    const matchLeida =
      filtroLeida === 'todas' ||
      (filtroLeida === 'leidas' && n.leida) ||
      (filtroLeida === 'no_leidas' && !n.leida)
  ...
  function openEdit(n) {
    setEditTarget(n)
    setForm({ titulo: n.titulo, mensaje: n.mensaje, leida: n.leida, fecha: n.fecha, usuario: n.usuario })
    setShowModal(true)
  }
  ...
  function handleSave() {
    ...
    if (editTarget) {
      setNotificaciones(prev => prev.map(n => n.id === editTarget.id ? { ...n, ...form } : n))
    } else {
      setNotificaciones(prev => [...prev, { ...form, id: Date.now() }])
    }
    setShowModal(false)
  }
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Gestion de notificaciones enviadas a los usuarios</p>
        </div>
      ...
      <div className="flex flex-wrap gap-3 mb-4">
        <input
          type="text"
          placeholder="Buscar por titulo, mensaje o usuario..."
          value={search}
            ...
            {filtered.length === 0 ? (
              ...
              filtered.map(n => (
                <tr key={n.id} className="bg-white dark:bg-gray-900 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                  ...
                  <td className="px-4 py-3 text-gray-600 dark:text-gray-300">{n.usuario}</td>
      ...
      <p className="mt-3 text-xs text-gray-400 dark:text-gray-500">
        {filtered.length} notificacion{filtered.length !== 1 ? 'es' : ''} encontrada{filtered.length !== 1 ? 's' : ''}
      </p>
        ...
        <Modal title={editTarget ? 'Editar notificacion' : 'Nueva notificacion'} onClose={() => setShowModal(false)}>
          <div className="flex flex-col gap-4">
            ...
            <div className="flex justify-end gap-3 pt-2">
              ...
              <button onClick={handleSave} className="px-4 py-2 text-sm rounded-md bg-blue-600 hover:bg-blue-700 text-white transition-colors">{editTarget ? 'Guardar cambios' : 'Crear notificacion'}</button>
            </div>
          </div>
        </Modal>
      )}
      ...
      {confirmId && (
        <ConfirmDialog
          message="Esta accion eliminara la notificacion permanentemente. Deseas continuar?"
