...
export default function TiposEstablecimiento() {
  ...
  async function handleSave() {
    ...
    try {
      if (editTarget) {
        await api.patch(`/api/admin/tipos-establecimiento/${editTarget.id}/`, form)
      } else {
        await api.post('/api/admin/tipos-establecimiento/', form)
      }
  ...
  async function handleDelete() {
    try {
      await api.delete(`/api/admin/tipos-establecimiento/${confirmId}/`)
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-semibold text-gray-800 dark:text-gray-100">Tipos de Establecimiento</h1>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Categorias de talleres y centros de servicio</p>
        </div>
